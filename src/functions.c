...
uint8_t *calculate_the_eap_md5_value_in_response_md5_challenge_frame(
    const uint8_t id, const uint8_t *eap_request_md5_chag_value,
  ...
  unsigned int i = 0;
  unsigned int md5_algo_input_array_length =
      sizeof(id) + password_length + eap_md5_value_size;

  md5_algo_input_array =
      (char *)malloc(md5_algo_input_array_length * sizeof(char));
  calculation_result = (char *)malloc(eap_md5_value_size * sizeof(char));
  uint8_type_calculation_result =
      (uint8_t *)malloc(eap_md5_value_size * sizeof(uint8_t));
  ...
  memcpy((md5_algo_input_array + sizeof(id)), password,
         password_length);  // cp password to input array
  memcpy(md5_algo_input_array + sizeof(id) + password_length,
         eap_request_md5_chag_value,
         eap_md5_value_size);  // cp eap_request_md5_chag_value to input array

  // use the funciton get_md5_digest to calculate the EAP-MD5 Value in
  // eap_response_md5_chag frame
  memcpy(calculation_result,
         get_md5_digest(md5_algo_input_array, md5_algo_input_array_length),
         eap_md5_value_size);
...
int code_convert(char *from_charset, char *to_charset, char *inbuf,
  ...
  if ((cd = iconv_open(to_charset, from_charset)) == (iconv_t)-1) return -1;
...
void print_notification_msg(const uint8_t *frame) {
  ...
  code_convert("gb2312", "utf-8", msg, msg_length, msg_buf, 1024);
...
void print_server_info(const uint8_t *frame) {
  ...
  if (msg_length) {
    msg = (char *)(frame + 0x1c);
    code_convert("gb2312", "utf-8", msg, msg_length, msg_buf, 1024);
    if (strlen(msg_buf) > 0) fprintf(stdout, ">>Ruijie 通知: %s\n", msg_buf);
  }
  ...
  if (msg_length) {
    ...
    code_convert("gb2312", "utf-8", msg, msg_length, msg_buf, 1024);
    if (strlen(msg_buf) > 0) fprintf(stdout, ">>账户信息: %s\n", msg_buf);
  }
...
int midnight_relogin() {
  ...
  user_input_ipaddr =
      NULL;  // clean user_input_ipaddr, to get the network card real ip addr
  ...
  if (init_NIC_address(&nic_info) ==
      -1) {  // get network interface card MAC, ip, mask
    int errnum = errno;
...
int relogin_when_receive_failure_frame() {
  ...
  if (service_company_bp != NULL) {
    if (relogin_number) free(service_company);
...
void HandleSigalrm(int sig, siginfo_t *siginfo, void *context) {
  ...
  switch (current_state) {
    ...
    case EAP_FAILURE:
      pcap_breakloop(
          descr);  // force pcap_loop() function return rather than looping
      break;

    case EAPOL_START:
      fprintf(stderr,
              "已发送认证信息，但未收到服务器响应包，请检查网络连接是否正常\n");
      pcap_breakloop(
          descr);  // force pcap_loop() function return rather than looping
      break;
...
//    初始化锁文件参数
//------------------------------------------------------------------------
int LockRegister(int fd, int cmd, int type, off_t offset, int whence,
                 off_t len) {
  struct flock lock;
...
//------------------------------------------------------------------------
int KillJMUSupplicant(int exit_flag, int fd, int type, off_t offset, int whence,
                      off_t len) {
  pid_t pid;
  if (exit_flag) {
...
//------------------------------------------------------------------------
int initiative_exit_program_with_already_running_check(int exit_flag, int fd,
                                                       pid_t pid) {
  if (exit_flag) {
    if (pid > 0) {
