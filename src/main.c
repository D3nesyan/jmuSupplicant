...
static void ShowUsage() {
  printf(
      ...
      "\t--interface_card      指定 "
      "网卡名称，则程序中不再要求用户手动选择网卡\n\n"
      "\t-n                    开启 midnight 模式,断网后使用。\n\n"

      "\t-b, --background      认证成功后,jmuSupplicant进入 dameon "
      "模式,后台运行。\n\n"
      ...
      "\n"
      "  关于 jmuSupplicant:\n\n"
      "\tjmuSupplicant is a program developed individually by 入山 and release "
      "under Apache-v2.0 \n"
      "\tlicense as free software, with NO any relaiontship with Ruijie "
      "company.\n\n\n"
...
static void InitArguments(int *argc, char ***argv) {
  ...
  while (1) {
    ...
    int option_index = 0;
    c = getopt_long((*argc), (*argv), "u:s:p:g:d:hbkmni", long_options,
                    &option_index);
    if (c == -1) break;
    switch (c) {
      case 0:
      ...
      case 'i':
        network_interface_card_name =
            optarg;  // 网络设备信息结构体,包含 ip, netmask, gateway 等等
        break;
      case 'h':
        ShowUsage();
        exit(EXIT_SUCCESS);
