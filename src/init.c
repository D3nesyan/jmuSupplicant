...
void init_login_config() {
  ...
  if (network_interface_card_name != NULL) {
    ...
    memcpy(nic_info.nic_name, network_interface_card_name,
           sizeof(nic_info.nic_name));
  }

#if defined(DEBUG)
  fprintf(stdout,
          "the received password from user input is : %s\n"
...
void init_device() {
  if (network_interface_card_name == NULL &&
      nic_info.nic_name != NULL) {  // specify a network interface card
    memset(nic_info.nic_name, '\0', sizeof(NIC_SIZE * sizeof(char)));
...
static void init_service_name() {
  if (service_company == NULL) {  // 用户未输入服务名,默认以'教育网'登录
    service_name =
        (uint8_t *)malloc(DEFAULT_SERVICE_NAME_SIZE * sizeof(uint8_t));
    ...
    switch (integer_type_service_company) {
      ...
      case 3:
        service_name =
            (uint8_t *)malloc(DEFAULT_SERVICE_NAME_SIZE * sizeof(uint8_t));
        memset(service_name, 0x00, DEFAULT_SERVICE_NAME_SIZE);
        memcpy(service_name, SERVICE_CHINA_TELECOM,
               SERVICE_CHINA_TELECOM_LENGTH);
        break;
    }
  }
...
int init_NIC_name(NIC_STRUCT *nic_info) {
  ...
  size_t available_adapter_num = 0,
         choosed_adapter = 0;  // 可选的网络设备数量; 被选择的网络设备序号
  char **available_adapter_name = NULL,
       **more_available_adapter_names =
           NULL;  // 可选的网络设备名; realooc()函数需要用到的指针
  ...
  for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++) {
    ...
    if (family == AF_INET) {  // only use ipv4
      s = getnameinfo(ifa->ifa_addr,
                      ...
                      hbuf, NIC_MAXHOST, NULL, 0, NI_NUMERICHOST);
      if (s != 0) {
        continue;
      } else {
        if (strlen(ifa->ifa_name) >=
            IFNAMSIZ)  // avoid adapter name exceed sizeof( ifr.ifr_name )
          continue;
        ...
        if (available_adapter_name == NULL) {
          ...
          available_adapter_name[(int)available_adapter_num - 1] =
              (char *)calloc(IFNAMSIZ, sizeof(char));
          if (available_adapter_name[(int)available_adapter_num - 1] == NULL) {
            printf(
                "Error (re)allocating adapters name memory "
                "(available_adapter_name[0])");
            exit(EXIT_FAILURE);
          }
        } else {
          more_available_adapter_names = (char **)realloc(
              available_adapter_name, available_adapter_num * sizeof(char *));
          if (more_available_adapter_names != NULL) {
            available_adapter_name = more_available_adapter_names;
            available_adapter_name[(int)available_adapter_num - 1] =
                (char *)calloc(IFNAMSIZ, sizeof(char));
            if (available_adapter_name[(int)available_adapter_num - 1] ==
                NULL) {
              printf(
                  "Error (re)allocating adapters name memory "
                  "(available_adapter_name[%zu])",
                  available_adapter_num - 1);
              exit(EXIT_FAILURE);
            }
        ...
        strcpy(available_adapter_name[available_adapter_num - 1],
               ifa->ifa_name);  // strcpy() the device name
        printf("[ %zu ] %s  <%s>\n", available_adapter_num, ifa->ifa_name,
               hbuf);  // 打印到控制台供用户选择输入
      }
    } else {
      continue;
  ...
  if (available_adapter_num == 0) {
    ...
    printf("please enter 1 ~ %zu to choose a network adapter:  ",
           available_adapter_num);
    ...
    // 存储用户选择的网络设备名
    memcpy(nic_info->nic_name, available_adapter_name[choosed_adapter],
           sizeof(nic_info->nic_name));
  }
...
int init_NIC_address(NIC_STRUCT *nic_info) {
  ...
  size_t if_name_len =
      strlen(if_name);  // ifr_name is a fixed-length buffer, be care about it
  int fd = socket(AF_UNIX, SOCK_DGRAM,
                  0);  // function ioctl() need a socket descriptor as variable
  ...
  printf("MAC address: %02X:%02X:%02X:%02X:%02X:%02X\n", host_MAC[0],
         host_MAC[1], host_MAC[2], host_MAC[3], host_MAC[4], host_MAC[5]);
#endif
