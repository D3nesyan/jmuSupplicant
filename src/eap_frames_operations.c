...
pcap_t *descr =
    NULL;  // PCAP packet capture descriptor for the specified interface.
...
int capture_eap_frames() {
  ...
  // compile the program, filter frames which contains ethernet interface MAC
  // address
  snprintf(filter_exp, sizeof(filter_exp), filter_exp_format,
           nic_info.nic_MAC[0], nic_info.nic_MAC[1], nic_info.nic_MAC[2],
           nic_info.nic_MAC[3], nic_info.nic_MAC[4],
           nic_info.nic_MAC[5]);  // FILTER_STR ether[12:2] = 0x888e and ether
                                  // dst "ether interface MAC"
  ...
  // 根据捕获到的服务器发送的不同类型的 Request EAP Frame,回应对应的 EAP frame
  // 用以认证
...
static void type_failure_action(const uint8_t *frame) {
  ...
  if (relogin_when_receive_failure_frame() != 0) {
    fprintf(stdout,
            "Attemp to relogin after program received failure frame failed.\n");
    exit(1);
  }
...
static void type_success_action(const uint8_t *frame) {
  ...
  if (!midnight && background_running) {
    daemon(1,
           0);  // create child process force the program running on background
  ...
  if (midnight == 1) {
    ...
    pcap_breakloop(descr);  // exit pcap capture process
...
int send_eap_frame(SEND_FRAME_TYPE type, const uint8_t *frame) {
  ...
  //  uint8_t eap_request_md5_value[EAP_MD5_VALUE_LENGTH]; //
  //  该hex数组从服务器发送的 request-md5-challenge frame 中提取 uint8_t eap_id
  //  = 0x01; // default 0x01, 详见 RFC 3748
  extern unsigned int midnight;

  switch (type) {
    case EAPOL_START:
      //    ethernet_header = (struct ether_header*)(frame); // extract ethernet
      //    header values
    ...
    case EAP_REQUEST_MD5_CHALLENGE:
      ...
      //    construct_eap_response_md5_challenge_frame(ethernet_header->ether_shost,
      //    eap_request_md5_value, eap_md5_value_size, frame);
    ...
    case EAP_SUCCESS:
      ...
      //    ethernet_header = (struct ether_header*)(frame); // extract ethernet
      //    header values
      //    construct_eapol_heartbeat_frame(ethernet_header->ether_shost,
      //    frame);
  ...
  char pcap_errbuf[PCAP_ERRBUF_SIZE];
  pcap_t *descr =
      pcap_open_live(nic_info.nic_name, SNAP_LENGTH, 1, 1000, pcap_errbuf);
  if (descr == NULL) {
    fprintf(stderr, "%s", pcap_errbuf);
  }
  if (!descr) {
  ...
  if (pcap_inject(descr, send_eap_frame_data, send_eap_frame_data_length) ==
      -1) {
